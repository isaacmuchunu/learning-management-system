  ...
  return (
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <h3 className="text-xl font-bold text-white mt-2 mb-3 group-hover:text-cyber-400 transition-colors line-clamp-2">
          {course.title}
        </h3>
        <p className="text-dark-400 text-sm leading-relaxed mb-5 line-clamp-3 flex-grow">
          {course.description}
        </p>
        ...
        <div className="flex flex-wrap gap-2 mb-5">
          {course.skills.slice(0, 3).map((skill) => (
            <span
              key={skill}
              className="px-3 py-1.5 text-xs bg-dark-800 text-dark-300 rounded-md"
            >
              {skill}
            </span>
          ))}
          {course.skills.length > 3 && (
            <span className="px-3 py-1.5 text-xs bg-dark-800 text-cyber-400 rounded-md">
              +{course.skills.length - 3}
            </span>
          )}
        </div>
        ...
        <div className="flex items-center justify-between pt-5 border-t border-dark-700 mt-auto">
          ...
          <Link
            ...
            className="group/btn inline-flex items-center gap-2 px-4 py-2.5 bg-cyber-500/10 text-cyber-400 font-medium rounded-lg border border-cyber-500/30 hover:bg-cyber-500 hover:text-dark-950 transition-all duration-300"
          >
